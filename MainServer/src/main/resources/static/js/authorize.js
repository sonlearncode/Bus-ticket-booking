// http://localhost:8088/busbooking/auth/login
...
function loginEvent(form) {
    if (form) {
        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                const response = await fetch('http://localhost:8088/busbooking/auth/login', {
                    method: 'POST',
                    headers: {
                ...
                if (response.ok && data.code === 0 && data.result.authenticated) {
                    ...
                    fetch('http://localhost:8088/busbooking/busticket/session-info', {
                        method: "GET",
                        credentials: "include"
                    })
...
function signUpEvent(form) {
    if (form) {
        form.addEventListener('submit', async (event) => {
            ...
            const response = await fetch('http://localhost:8088/busbooking/users', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
